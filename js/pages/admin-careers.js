...
const{data,error}=await dbAdmin.from('careers').select('id,title,department,location,deadline,is_active').order('created_at',{ascending:false});
...
'<td style="padding:.75rem;font-size:.875rem;color:#6b7280;">'+(j.location||'')+'</td>'+
'<td style="padding:.75rem;"><span style="font-size:.6875rem;font-weight:600;padding:.2rem .5rem;border-radius:2rem;background:'+(j.is_active?'#f0f7f1':'#f3f4f6')+';color:'+(j.is_active?'#1b5e20':'#6b7280')+'">'+(j.is_active?'Active':'Inactive')+'</span></td>'+
'<td style="padding:.75rem;font-size:.75rem;color:#9ca3af;">'+(j.deadline?new Date(j.deadline).toLocaleDateString():'')+'</td>'+
...
async function editItem(id){const{data}=await dbAdmin.from("careers").select("*").eq("id",id).single();openModal(data);}
async function deleteItem(id){if(!confirm("Delete this record?"))return;await dbAdmin.from("careers").delete().eq("id",id);loadItems();}
...
const payload={title:fd.get('title'),department:fd.get('department'),location:fd.get('location'),deadline:fd.get('deadline')||null,description:fd.get('description'),is_active:fd.get('is_active')==='on'};
const{error}=editId?await dbAdmin.from("careers").update(payload).eq("id",editId):await dbAdmin.from("careers").insert([payload]);
